...
void drawBorders() {
    // 37 = White, 44 = Blue Background
    printf("\033[37;44m");

    COLOURS baground_colour = BG_BLUE;

    // Top and Bottom Borders
    for (int x = 0; x < width; x++) {
        drawCharAt(x, 0, ' ', baground_colour);            
        drawCharAt(x, height - 1, ' ', baground_colour);   
    }

    // Left and Right Borders
    for (int y = 0; y < height; y++) {
        drawCharAt(0, y, ' ', baground_colour);            
        drawCharAt(width - 1, y, ' ', baground_colour);    
    }
